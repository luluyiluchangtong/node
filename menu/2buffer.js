...
// Buffer 类 被引入处理(读取 或 操作) 二进制数据 (在 TCP流 和 文件系统操作等场景中) ES6 的TypedArray 也是处理二进制数据的
...
// 创建 Buffer 实例
// 使用 Buffer.alloc   Buffer.allocUnsafe  Buffer.allocUnsafeSlow()  Buffer.from() 代替 new Buffer() 构造函数

// 1. Buffer.alloc()
const bf = Buffer.alloc(10, 44, 'utf-8')
console.log(bf) // 长度为 10； 44 填充（默认是 0）；字符编码为 'utf-8'
console.log(bf[2]) // 44 返回指定索引处的 字节 
...
console.log(buf);
// 虽然比Buffer.alloc（）快，但每次都 未初始化内存，因此之后每次输出都包含旧数据，可能包含敏感数据， 不安全！！
