...
fs.unlink('文件', (err) => {
    // fs 接受的 路径格式有： 
    // 1.字符串(相对路径)  'file.txt'  (绝对路径) '/file.txt'
    // 2.Buffer(相对路径，绝对路径)  Buffer.from('/open/some/file.txt')  
    // 3.File: 协议的 url 对象(必须携带主机名 或 驱动器号(C 盘，D盘...))  
    // 主机名   new URL('file://DESKTOP-CADONJJ/desktop/node-note/menu/fs2.js')
    // 驱动器号 new URL('file:///C:/tmp/hello')
    
    if (err) throw err;
    // 异步操作文件时， 最后一个参数是  完成时的 '回调函数' ， 函数的第一个参数保留给异常，
    console.log('已成功地删除文件');
    })
    
    // 异步的方法 fs.rename()  fs.stat()不能保证执行顺序, 所以需要嵌套执行
